...
class IRnet_Multimodal_Transformer(torch.nn.Module):
    ...
    def cls_loss(self,pre,ans_label):
        ...
        ans_acc = hit/bs
        return ans_loss,ans_acc
class IRnet_Multimodal_Transformer_wto_Ws(torch.nn.Module):
    def __init__(self, picture_dim=512, hidden_dim=768, device = "cuda:0"):
        super(IRnet_Multimodal_Transformer_wto_Ws, self).__init__()
        self.device = device
        self.picture_linear = nn.Linear(picture_dim, hidden_dim, bias=False)  # 512 * 128
        # multi-layers transformer blocks, deep network
        self.Self_transformer_blocks = nn.ModuleList(
    ...
    def cls_loss(self,pre,ans_label):
        ...
        ans_acc = hit/bs
        return ans_loss,ans_acc
class IRnet_Multimodal_Transformer_wto_Wt(torch.nn.Module):
    def __init__(self, picture_dim=512, hidden_dim=768, device = "cuda:0"):
        super(IRnet_Multimodal_Transformer_wto_Wt, self).__init__()
        self.device = device
        self.picture_linear = nn.Linear(picture_dim, hidden_dim, bias=False)  # 512 * 128
        # multi-layers transformer blocks, deep network
        self.Self_transformer_blocks = nn.ModuleList(
    ...
    def cls_loss(self,pre,ans_label):
        ...
        ans_acc = hit/bs
        return ans_loss,ans_acc
class IRnet_Multimodal_Transformer_wto_Wc(torch.nn.Module):
    def __init__(self, picture_dim=512, hidden_dim=768, device = "cuda:0"):
        super(IRnet_Multimodal_Transformer_wto_Wc, self).__init__()
        self.device = device
        self.picture_linear = nn.Linear(picture_dim, hidden_dim, bias=False)  # 512 * 128
        # multi-layers transformer blocks, deep network
        self.Self_transformer_blocks = nn.ModuleList(
    ...
    def cls_loss(self,pre,ans_label):
        ...
        ans_acc = hit/bs
        return ans_loss,ans_acc
class IRnet_Multimodal_Transformer_wto_retrieval(torch.nn.Module):
    def __init__(self, picture_dim=512, hidden_dim=768, device = "cuda:0"):
        super(IRnet_Multimodal_Transformer_wto_retrieval, self).__init__()
        self.device = device
        self.picture_linear = nn.Linear(picture_dim, hidden_dim, bias=False)  # 512 * 128
        # multi-layers transformer blocks, deep network
        self.Self_transformer_blocks = nn.ModuleList(
    ...
    def forward(self,batch_text,batch_image,que_mask,ans,ans_mask,label,ans_label,b_s,c_s,train_flag):
        ...
        ans_loss,ans_acc = self.Answer_model(query.detach(),gt_ans.detach(),gt_ans_mask,ans_label)
        return 0,0,ans_loss,ans_acc  # 【batch,candidate_num】
    ...
    def cls_loss(self,pre,ans_label):
        ...
        ans_acc = hit/bs
        return ans_loss,ans_acc
class IRnet_Multimodal_Transformer_wto_QMKRinretrieval(torch.nn.Module):
    def __init__(self, picture_dim=512, hidden_dim=768, device = "cuda:0"):
        super(IRnet_Multimodal_Transformer_wto_QMKRinretrieval, self).__init__()
        self.device = device
        self.picture_linear = nn.Linear(picture_dim, hidden_dim, bias=False)  # 512 * 128
        # multi-layers transformer blocks, deep network
        self.Self_transformer_blocks = nn.ModuleList(
    ...
    def forward(self, batch_text, batch_image, que_mask, ans, ans_mask, label, ans_label, b_s, c_s, train_flag):
        ...
        if train_flag:  # 在训练和验证的时候保持一致
            ...
            highest_index = torch.sort(coo, descending=True)[1][:, :self.top_k]  # [bs,5]
            gt_ans = torch.stack([torch.index_select(ans[i], 0, highest_index[i]) for i in range(b_s)])  # [k,attr,dim]
            gt_ans_mask = torch.stack(
                [torch.index_select(ans_mask[0][i], 0, highest_index[i]) for i in range(b_s)])  # [k,attr]
            # gt_ans = ans
            # gt_ans_mask = ans_mask[0]
    ...
    def Multimodal_Trans(self,Q_r,ans_i,que_mask,ans_mask_i):
        ...
        # 线索和query融合 打分  这里不需要mask吗？
        for transformer in self.Cross_transformer_blocks:
            clue,Q_r = transformer.forward(clue,Q_r,ans_mask_i,que_mask)  # [b,2,dim] [b,2,dim]
    ...
    def cls_loss(self,pre,ans_label):
        ...
        ans_acc = hit/bs
        return ans_loss,ans_acc
class IRnet_Multimodal_Transformer_wto_QMKRinanswer(torch.nn.Module):
    def __init__(self, picture_dim=512, hidden_dim=768, device = "cuda:0"):
        super(IRnet_Multimodal_Transformer_wto_QMKRinanswer, self).__init__()
        self.device = device
        self.picture_linear = nn.Linear(picture_dim, hidden_dim, bias=False)  # 512 * 128
        # multi-layers transformer blocks, deep network
        self.Self_transformer_blocks = nn.ModuleList(
    ...
    def Answer_model(self,query,ans,ans_mask,ans_label):
        ...
        cls = torch.zeros([query.shape[0],1,query.shape[2]]).to(self.device)
        x = torch.cat((cls, query, ans), dim=1)
        # que_mask = torch.ones([query.shape[0],1+2+ans.shape[1]]).to(self.device)
        que_mask = torch.ones([query.shape[0], 1 + 2]).to(self.device)
        que_mask = torch.cat([que_mask,ans_mask],dim=-1)
    ...
    def cls_loss(self,pre,ans_label):
        ...
        ans_acc = hit/bs
        return ans_loss,ans_acc
class IRnet_Multimodal_Transformer_wto_MF(torch.nn.Module):
    def __init__(self, picture_dim=512, hidden_dim=768, device = "cuda:0"):
        super(IRnet_Multimodal_Transformer_wto_MF, self).__init__()
        self.device = device
        self.picture_linear = nn.Linear(picture_dim, hidden_dim, bias=False)  # 512 * 128
        # multi-layers transformer blocks, deep network
        self.Self_transformer_blocks = nn.ModuleList(
    ...
    def Multimodal_Trans(self,Q_r,ans_i,que_mask,ans_mask_i):
        ...
        #
        sco1 = torch.sum(clue[:,0,:] * Q_r[:,0,:],dim=-1)
        sco2 = torch.sum(clue[:, 1, :] * Q_r[:, 1, :], dim=-1)
        return sco1+sco2
    def Answer_model(self,query,ans,ans_mask,ans_label):
        # query（bs,2,128）  ans(bs,attr,128)  mask(bs,attr)
        # 用一个transformer整合一下
    ...
    def cls_loss(self,pre,ans_label):
        ...
        ans_acc = hit/bs
        return ans_loss,ans_acc
class IRnet_Multimodal_Transformer_wto_FF(torch.nn.Module):
    def __init__(self, picture_dim=512, hidden_dim=768, device = "cuda:0"):
        super(IRnet_Multimodal_Transformer_wto_FF, self).__init__()
        self.device = device
        self.picture_linear = nn.Linear(picture_dim, hidden_dim, bias=False)  # 512 * 128
        # multi-layers transformer blocks, deep network
        self.Self_transformer_blocks = nn.ModuleList(
        ...
        self.lstm_enc_type = EncoderRNN(vocab_size = 7, embed_size = hidden_dim, hidden_size = hidden_dim,
                                        ...
                                        device = device)
        self.loss_fn = MultiLabelMarginLoss()
        self.prediction = SimpleClassifier(hidden_dim*4, hidden_dim*2, 2, 0.5)
        self.cls_loss_fn = nn.CrossEntropyLoss()
        self.top_k = 16
    ...
    def cls_loss(self,pre,ans_label):
        ...
        return ans_loss,ans_acc
#########################################################################
class IRnet_Multimodal_Transformer_jk(torch.nn.Module):
    def __init__(self, cfg, device = "cuda:0"):
        super(IRnet_Multimodal_Transformer_jk, self).__init__()
        ...
        # 检索
        self.Single_transformer_blocks = nn.ModuleList(
            [SingleTransformerBlock(hidden=hidden_dim, attn_heads=4, feed_forward_hidden=hidden_dim*2, dropout=0) for _ in range(cfg["MODEL"]["K"])])
        self.Cross_transformer_blocks = nn.ModuleList(
            [CrossTransformerBlock(hidden=hidden_dim, attn_heads=4, feed_forward_hidden=hidden_dim*2, dropout=0) for _ in range(cfg["MODEL"]["J"])])
        self.lstm_enc_type = EncoderRNN(vocab_size = 7, embed_size = hidden_dim, hidden_size = hidden_dim,
                                        bidirectional=True,
                                        rnn_type='lstm',
                                        device = device)
    ...
    def cls_loss(self,pre,ans_label):
        ...
        ans_acc = hit/bs
        return ans_loss,ans_acc
class IRnet_Multimodal_Transformer_mn(torch.nn.Module):
    def __init__(self, cfg, device="cuda:0"):
        super(IRnet_Multimodal_Transformer_mn, self).__init__()
        hidden_dim = cfg["MODEL"]["hidden_dim"]
        picture_dim = cfg["MODEL"]["picture_dim"]
        self.device = device
        self.picture_linear = nn.Linear(picture_dim, hidden_dim, bias=False)  # 512 * 128
        self.Self_transformer_blocks = nn.ModuleList(
            [TransformerBlock(hidden=hidden_dim, attn_heads=4, feed_forward_hidden=hidden_dim*2, dropout=0) for _ in range(cfg["MODEL"]["N"])])
        self.Single_transformer_blocks_for_ans = nn.ModuleList(
            [SingleTransformerBlock(hidden=hidden_dim, attn_heads=4, feed_forward_hidden=hidden_dim*2, dropout=0) for _ in range(cfg["MODEL"]["M"])])
        # 检索
        self.Single_transformer_blocks = nn.ModuleList(
            [SingleTransformerBlock(hidden=hidden_dim, attn_heads=4, feed_forward_hidden=hidden_dim*2, dropout=0) for _ in range(2)])
        self.Cross_transformer_blocks = nn.ModuleList(
